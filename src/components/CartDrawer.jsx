      ...
      <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {cartItems.length === 0 ? (
            <div className="text-center py-20">
              ...
              <button onClick={() => setIsCartOpen(false)} className="text-brand-DEFAULT underline text-sm font-bold uppercase tracking-widest">Start Shopping</button>
            </div>
          ) : (
            cartItems.map((item) => (
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-slate-100 bg-slate-50">
            ...
            <Link 
                to="/checkout" 
                onClick={() => setIsCartOpen(false)} // Close drawer when clicking
                className="block w-full bg-slate-900 text-white py-4 text-center text-sm font-bold uppercase tracking-[0.2em] hover:bg-brand-DEFAULT transition-colors"
                >
                Proceed to Checkout
            </Link>
          </div>
